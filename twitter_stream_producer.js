...
function twitterStreamProducer(kinesis) {
  ...
  function _createStreamIfNotCreated(callback) {
    kinesis.describeStream({StreamName: config.kinesis.StreamName, ShardCount: 2,region: 'us-east-1'}, function(err, data) {
      if (err) {
        kinesis.createStream(config.kinesis, function(err, data) {
          if (err) {
            ...
            if (err.code !== 'ResourceInUseException') {
              ...
              var msg = util.format('%s stream is already created! Re-using it.', config.kinesis.StreamName);
              console.log(msg);
              log.info(msg);
            }
          }
          else {
            var msg = util.format('%s stream does not exist. Created a new stream with that name.', config.kinesis.StreamName);
            console.log(msg);
            log.info(msg);
          }
  ...
  function _waitForStreamToBecomeActive(callback) {
    kinesis.describeStream({StreamName: config.kinesis.StreamName}, function(err, data) {
      if (!err) {
        if (data.StreamDescription.StreamStatus === 'ACTIVE') {
          ...
          var msg = util.format('Current status of the stream is %s.', data.StreamDescription.StreamStatus);
          console.log(msg);
          log.info(msg);
          setTimeout(function() {
            _waitForStreamToBecomeActive(callback);
